...
func TestUnmanagedFunction_Metadata(t *testing.T) {
	f := NewBigdataSaasUnmanagedFunction()
	req := function.MetadataRequest{}
	resp := &function.MetadataResponse{}
	f.Metadata(context.Background(), req, resp)
	if resp.Name != "bigdata_saas_unmanaged" {
		t.Errorf("expected name %q, got %q", "bigdata_saas_unmanaged", resp.Name)
	}
}
...
func TestUnmanagedFunction_Run(t *testing.T) {
	...
	if ct := attrs["type"].(types.String); ct.ValueString() != "BigData.SaaS.Unmanaged" {
		t.Errorf("expected type %q, got %q", "BigData.SaaS.Unmanaged", ct.ValueString())
	}
