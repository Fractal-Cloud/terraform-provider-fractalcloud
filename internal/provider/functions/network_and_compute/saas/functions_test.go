...
func TestUnmanagedFunction_Metadata(t *testing.T) {
	...
	if resp.Name != "network_and_compute_saas_unmanaged" {
		t.Errorf("expected name %q, got %q", "network_and_compute_saas_unmanaged", resp.Name)
	}
}
...
func TestUnmanagedFunction_Run(t *testing.T) {
	...
	if ct := attrs["type"].(types.String); ct.ValueString() != "NetworkAndCompute.SaaS.Unmanaged" {
		t.Errorf("expected type %q, got %q", "NetworkAndCompute.SaaS.Unmanaged", ct.ValueString())
	}
