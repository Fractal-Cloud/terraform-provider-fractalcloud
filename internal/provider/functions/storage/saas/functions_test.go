...
func TestUnmanagedFunction_Metadata(t *testing.T) {
	f := NewStorageSaasUnmanagedFunction()
	req := function.MetadataRequest{}
	resp := &function.MetadataResponse{}
	f.Metadata(context.Background(), req, resp)
	if resp.Name != "storage_saas_unmanaged" {
		t.Errorf("expected name %q, got %q", "storage_saas_unmanaged", resp.Name)
	}
}
...
func TestUnmanagedFunction_Run(t *testing.T) {
	...
	if ct := attrs["type"].(types.String); ct.ValueString() != "Storage.SaaS.Unmanaged" {
		t.Errorf("expected type %q, got %q", "Storage.SaaS.Unmanaged", ct.ValueString())
	}
