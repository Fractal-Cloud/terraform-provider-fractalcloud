...
func TestUnmanagedFunction_Metadata(t *testing.T) {
	...
	if resp.Name != "api_management_saas_unmanaged" {
		t.Errorf("expected name %q, got %q", "api_management_saas_unmanaged", resp.Name)
	}
}
...
func TestUnmanagedFunction_Run(t *testing.T) {
	...
	if ct := attrs["type"].(types.String); ct.ValueString() != "APIManagement.SaaS.Unmanaged" {
		t.Errorf("expected type %q, got %q", "APIManagement.SaaS.Unmanaged", ct.ValueString())
	}
